// Definición de la estructura de las preguntas y respuestas
interface Question {
    question: string;
    options: string[];
    correctAnswer: number; // El índice de la respuesta correcta
  }
  ...
  // Elementos del DOM
  const questionElement = document.getElementById('question') as HTMLSpanElement;
  const options: HTMLButtonElement[] = Array.from(document.querySelectorAll('.option-button')) as HTMLButtonElement[];
